...
y_train, y_valid, y_test, data_min, data_max = \
    load_timeseries(dataset, train_size, test_size, valid_size)
print(data_min, data_max)

model = Sequential()
model.add(Dense(units=layers[0], activation='tanh'))
model.add(Dense(units=layers[1], activation='tanh'))
...
# fit network
bpnn = model.fit(x_train, y_train,
                 epochs=epochs,
...
# plot history
plt.plot(bpnn.history['loss'], label='train')
plt.plot(bpnn.history['val_loss'], label='valid')
plt.legend()
plt.show()
